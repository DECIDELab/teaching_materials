...
date: " 2020-11-17 (cập nhật: `r Sys.Date()`)"
output:
    html_document:
      theme: cerulean
      toc: yes
...
the determination that an association is causal indicates the possibility for intervention

* Practitioners decide on interventions on the basis of consequences produced by a presumed causal relationship
* Causal inference is embedded in regulatory processes
* Public health evidence may be prominent in legal proceedings in which judgment about the existence of a causal relationship is pivotal in determining guilt and liability for damages
* Causal inference is also embedded in many aspects of medical practice through the principles of evidence-based medicine, where decisions about harms or benefits of therapeutic agents are based, in part, on rules for how
to measure the strength of evidence for causal connections between interventions and health outcomes

frameworks:

* John Snow “chains of inference”
* Koch’s postulates: germ theory
* Sir Austin Bradford Hill
* 1964 US Surgeon General’s report on smoking
* counterfactual or potential outcomes framework
...
classic framework for causal thinking

causal role of smoking in the epidemic of lung cancer

to identify the causes of diseases and particularly to determine the role of smoking in lung cancer but its use has been extended to public health decision making, a domain where questions about causal effects relate to the consequences of interventions that have often been motivated by the identification of causal factors

does not reflect the current, more clearly articulated view of causal processes. and the the causal questions have become more complex, beyond the original intent of this framework. lack of a formal basis for evaluating causal hypotheses

Hill’s criteria (1965):

* Strength
* Consistency
* Specificity: little applicability for noncommunicable diseases that have multiple causes
* Temporality
* Biological gradient
* Coherence
* Experiment

vulnerable to subjectivity in the evaluation of evidence and to manipulation of the evidence and evidence interpretation

assumes a simplistic direct relationship between cause and putative effect without explicit consideration of the structure of the underlying causal processes

limited focus: The inference about cause became the rationale for intervention, but the causal conclusions were not couched in the consequences of specific actions to reduce or eliminate cigarette smoking. And later, public health action was aimed at the individual smoker, rather than at the upstream system of cigarette manufacture, advertising, and distribution

causal determinations were made by epidemiologists and others in public health about various risk factors without considering the effect of a specific way of changing them

### US Surgeon General Report’s criteria
...
From the course Causal questions and principled answers: a guide through the landscape for practising statisticians.

## A causal effect question for a purpose: the steps
...
4. Formalise potential outcomes, one for each level of the treatment that the subject could have possibly experienced
5. Chose the causal target parameter or estimand , as a (summary) contrast between the potential outcome distributions.
6. State the assumptions validating that the causal effect can be estimated with the data at hand.
7. Estimation of ‘the’ causal effect
8. Evaluate assumptions, sensitivity analysis .

## Potential outcomes and causal estimands

### Introduction

### Causal questions for a purpose

Two general questions as a running thread throughout:

1. Does some form of breastfeeding (BF) action increase the babies weight at three months?
2. Is the use of a pulmonary artery catheter prolonging survival time for patients admitted to an intensive care unit (ICU) in critical condition?

Questions posed

* by the ‘patient’, care giver, health insurance, ... 
* for decisions
...
* Simulated on www.ofcaus.org (cfr. practical 2)

#### General notation for the observed data

* A for observed Action or Active exposure level
* Y for the observed outcome
* L for observed Lifestyle factors or baseline covariates

#### What can we learn? ITT

If A is randomised

* L−distribution common to all A−levels
* learn causal effect of training offer on weight@3m

Comparing (mean) outcomes between randomized arms 

$$E(Y|A = 1) − E(Y|A = 0)$$

measures a comparative causal effect...

A Randomised --> causal effect of training offer on weight@3m compares (mean) outcomes between randomized arms

* ACE: Average Causal Effect also called
* ATE: Average Treatment Effect

averaging over

* the entire study population
* or subpopulations of given measured baseline covariates L education level, health of the parents, job of the mother, ...

Estimate in PROBITsim: 94.2 grams (SE 9.1 grams) 
We may wish to know more.

### Observed exposures

#### Further questions on related exposure measures

Pregnant woman may ask about impact on weight@3m of:

1. following a BF program that is offered
2. starting BF
3. continuing BF for the full 3 months

in ‘her’ population stratum

Health insurer may ask about impact on weight@3m of:

1. offering a BF encouragement program
2. following the BF program
3. starting BF
4. continuing BF for a full 3 months

in different (targeted) population strata in the current or improved program?

#### The problem of observational data

Confounding: Different levels of observed exposure A are likely seen in population strata with different prognosis of outcome ‘under the reference exposure level (0)’

For instance:

* The group of women who is observed to breastfeed may differ from the group who is observed not to wrt. SES, education, health...

* Hence: comparing baby weight at 3 months between both groups compares results following a package of different circumstances, including breastfeeding exposure upto 3mo.

### Possible exposures and potential outcomes

What potential outcome (distribution) is expected following (two) different possible exposure levels

### Assumptions for causal inference

### Situating different exposure types

### Simulating the parallel worlds - a resource
...
Bệnh được lây truyền khi tác nhân rời bỏ vật chủ, hoặc túc chủ của nó, qua một ngõ ra, rồi được chuyển tải bằng một cách thức lây nào đó, và xâm nhập qua một ngõ vào thích hợp để gây nhiễm một túc chủ cảm thụ mới. Quá trình được gọi là **dây chuyền lây**.
...
* một sự xuất hiện có mô hình bất thường, với những trường hợp lẻ tẻ xảy ra vào những khoảng thời gian bất thường được gọi là __ca lẻ tẻ__
* một mức độ xuất hiện duy trì ở một mức bệnh thấp cho tới trung bình được xem là __mức lưu hành__ 
* một sự xuất hiện duy trì ở mức độ cao được gọi là __mức lưu hành cao__
* thỉnh thoảng, mức bệnh vượt hơn mức mong đợi, trong một vùng vào một thời điểm cụ thể, nó được gọi là một __vụ dịch__. Những nhân viên sức khoẻ cộng đồng thường dùng từ bùng phát, cùng một nghiã với dịch, nhưng từ bùng phát ít làm kinh động công chúng
* khi một trận dịch lan ra một vài quốc gia hoặc đại lục, ảnh hưởng một số lớn người, nó được gọi là __đại dịch__.
...
[Phiếu lượng giá trước buổi học](https://forms.office.com/Pages/ResponsePage.aspx?id=-Miz6oh0D0aitEEurd6ov5n02b8-tohPqXmBIVH6UQxUQkhCVkYzN0lZWEYzMlI0SzhCTlZCOTFTSy4u)

## Tài liệu tham khảo

Chương 2 sách Dịch tễ học cơ bản, bộ môn Dịch tễ học (Khoa Y tế công cộng, Đại học Y Dược TP Hồ Chí Minh), chủ biên: PGS. TS. Nguyễn Đỗ Nguyên.
